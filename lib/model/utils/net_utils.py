...
# 把类别名称对应成C1. C2, C3....
name_dict = {'baseballfield': 'C1', 'basketballcourt': 'C2', 'bridge': 'C3', 'chimney': 'C4', 'ship': 'C5',
             'airplane': 'C6', 'airport': 'C7', 'Expressway-toll-station': 'C8', 'harbor': 'C9', 'groundtrackfield': 'C10',
...
def vis_detections_label_only(im, class_name, dets, thresh=0.8):
    ...
    if class_name == '':
        for i in range(np.minimum(10, dets.shape[0])):
            ...
            score = dets[i, -1]
            if score > thresh:
                cv2.rectangle(im, bbox[0:2], bbox[2:4], (255, 255, 255), 2)
                text_size = cv2.getTextSize('%s' % (name_dict[class_name]), cv2.FONT_HERSHEY_COMPLEX, 0.6, 1)
                point = (bbox[0] + text_size[0][0], bbox[1] - text_size[0][1] - text_size[1])
                cv2.rectangle(im, bbox[0:2], point, (255, 255, 255), -1)
                cv2.putText(im, '%s' % (name_dict[class_name]), (bbox[0], bbox[1] - 8), cv2.FONT_HERSHEY_COMPLEX,
...
# 这里需要修改
def vis_rpn_proposals(im, dets, thres=0.8):
