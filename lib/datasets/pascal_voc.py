...
class pascal_voc(imdb):
    ...
    def gt_roidb(self):
        ...
        """
        # 如果不把这个注释掉，下一次运算会选择不同的图像，导致对应不上。
...
class dior(imdb):
    def __init__(self, image_set, devkit_path="/disk6/spz/dataset/DIOR/DIOR2007"):
        ...
        self._data_path = '/disk6/spz/dataset/DIOR/DIOR2007'
        # # 在phase 2和测试阶段记得改回来
        #first split
        if cfg.TRAIN.META_TYPE == 1:
            self._classes = ['__background__'] + cfg.TRAIN.ALLCLASSES_FIRST
        ...
        if cfg.TRAIN.META_TYPE == 3:
            self._classes = ['__background__'] + cfg.TRAIN.ALLCLASSES_THIRD
        #fourth split
        if cfg.TRAIN.META_TYPE == 4:
            self._classes = ['__background__'] + cfg.TRAIN.ALLCLASSES_FOURTH
        # 对比试验
        if cfg.TRAIN.META_TYPE == 5:
            self._classes = ['__background__'] + cfg.TRAIN.ALLCLASSES_FSODM

        # # #first split
        # if cfg.TRAIN.META_TYPE == 1:
        #     self._classes = ['__background__'] + cfg.TRAIN.BASECLASSES_FIRST
        # #second split
        # if cfg.TRAIN.META_TYPE == 2:
        #     self._classes = ['__background__'] + cfg.TRAIN.BASECLASSES_SECOND
        # #third split
        # if cfg.TRAIN.META_TYPE == 3:
        #     self._classes = ['__background__'] + cfg.TRAIN.BASECLASSES_THIRD
        # #fourth split
        # if cfg.TRAIN.META_TYPE == 4:
        #     self._classes = ['__background__'] + cfg.TRAIN.BASECLASSES_FOURTH
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            # 我不懂为什么要用小写的
            # if obj.find('name').text.lower().strip() not in self._classes:
            if obj.find('name').text.strip() not in self._classes:
    ...
    def _get_voc_results_file_template(self):
        # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
        filename = self._get_comp_id() + '_det_' + self._image_set + '_{:s}.txt'
        filedir = os.path.join(self._devkit_path, 'results', 'Dior', 'Main')
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        path = os.path.join(filedir, filename)
        return path
    ...
    def _do_python_eval(self, output_dir='output', **kwargs):
        ...
        for i, cls in enumerate(self._classes):
            ...
            # import pylab as pl
            # pl.plot(rec, prec, lw = 2, label = cls)
            # pl.xlabel('Recall')
            # pl.ylabel('Precision')
            # plt.grid(True)
            # pl.ylim([0.0, 1.05])
